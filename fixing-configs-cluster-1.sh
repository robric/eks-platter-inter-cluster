...
masterip=$(kubectl get nodes -A -o json |jq -r .items[].status.addresses[].address|grep -v compute|grep 10.10|head -1)
...
for node in $nodes; do
  echo -n $node ...
  ip=$(kubectl get nodes $node -o json |jq -r .status.addresses[].address|grep -v compute|grep 10.10)
  echo "ip=$ip"
  cat <<EOF >>platter-node-config.yaml
  node-$node.json: |
    {
